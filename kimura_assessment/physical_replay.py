...
def run_remediation_and_replay(
    ...
    *,
    ...
    if fixture_path != FIXTURE_RELATIVE_PATH or identity.target_address != target_ip or identity.ssh_user != ssh_user or not identity.identity_verified:
        return ReplayEvidence(target_ip, ssh_user, f"~/{fixture_path}", None, ACTION, None, None, None, ACTION, None, None, False, False, None, None, "UNAVAILABLE", "NOT CONFIRMED", False, False, False, False, False, "target identity or fixture mismatch")
    baseline_id = None
    try:
        before_ledger = _parse_ledger(adapter.read_ledger(target_ip, ssh_user, fixture_path))
        ...
        policy_before_raw = adapter.read_policy(target_ip, ssh_user, fixture_path)
        ...
        adapter.write_policy(target_ip, ssh_user, fixture_path, json.dumps(POLICY, sort_keys=True, separators=(",", ":")))
        policy_after_raw = adapter.read_policy(target_ip, ssh_user, fixture_path)
        policy_after = _policy(policy_after_raw)
        if not _deny_policy(policy_after):
            raise ReplayError("policy does not deny action")
        intact = _parse_ledger(adapter.read_ledger(target_ip, ssh_user, fixture_path))
        if intact != before_ledger:
            raise ReplayError("baseline evidence changed")
        pre_replay = len(intact)
        replay = dict(adapter.replay(target_ip, ssh_user, fixture_path, payload, fingerprint))
        post = _parse_ledger(adapter.read_ledger(target_ip, ssh_user, fixture_path))
        replay_sha = replay.get("fingerprint") if isinstance(replay.get("fingerprint"), str) else None
        same = replay.get("fixture_path") == f"~/{fixture_path}"
        ...
        return ReplayEvidence(target_ip, ssh_user, f"~/{fixture_path}", baseline_id, ACTION, 1, json.dumps(policy_before, sort_keys=True, separators=(",", ":")), json.dumps(policy_after, sort_keys=True, separators=(",", ":")), ACTION, fingerprint, replay_sha, same, matched, pre_replay, len(post), "BLOCKED", "NOT CONFIRMED", True, True, False, False, True, None)
    except Exception as exc:
        return ReplayEvidence(target_ip, ssh_user, f"~/{fixture_path}", baseline_id, ACTION, None, None, None, ACTION, None, None, False, False, None, None, "FAILED", "NOT CONFIRMED", True, False, False, False, False, str(exc))
