...
from .scenario_protocol import SP_V1_REGISTRY, ScenarioDefinition, ScenarioProtocolError, validate_evidence_binding
...
class PhysicalConferenceOrchestrator:
    ...
    def __init__(self, run_id: str, *, adapter: PhysicalLifecycleAdapter, emit, physical_run_id: str | None = None, scenario: ScenarioDefinition | None = None, scenario_id: str | None = None, scenario_version: int | None = None) -> None:
        ...
        if scenario is not None and (scenario_id is not None or scenario_version is not None):
            raise ValueError("use scenario or scenario_id/scenario_version, not both")
        if scenario_id is not None or scenario_version is not None:
            if scenario_id is None or scenario_version is None:
                raise ScenarioProtocolError("scenario_id and scenario_version are required together")
            scenario = SP_V1_REGISTRY.resolve(scenario_id, scenario_version)
        elif scenario is not None:
            if not isinstance(scenario, ScenarioDefinition):
                raise ValueError("scenario must be a ScenarioDefinition")
            scenario = SP_V1_REGISTRY.resolve(scenario.scenario_id, scenario.scenario_version)
    ...
    def start(self) -> ConferenceRunResult:
        ...
        stage = "discovery"
        self._emit_event(ProgressEventType.ASSESSMENT_STARTED, {"assessment_id": "physical-assessment-v1"})
        try:
            ...
            self._emit_event(ProgressEventType.TARGET_VERIFIED, {
    ...
    def _emit_event(self, event_type, payload):
        ...
        if self.scenario is not None:
            enriched.update(self.scenario.evidence_binding())
        self._emit(event_type, enriched)
    ...
    def _stage(self, operation) -> Mapping[str, Any]:
        ...
        if self.scenario is not None:
            validate_evidence_binding(evidence, self.scenario)
        return evidence
