...
(() => {
  ...
  function render() {
    ...
    const transformation = document.createElement("div"); transformation.className = "transformation"; const before = document.createElement("section"); before.className = "transform-card before"; text(before, "h2", "BEFORE"); text(before, "div", baseline.action || "NOT ESTABLISHED", "action"); text(before, "div", baseline.decision ? baseline.decision.toUpperCase() : "NOT ESTABLISHED", baseline.decision ? "decision" : "decision long-status"); text(before, "div", baseline.event_id ? "SYNTHETIC IMPACT CONFIRMED" : "IMPACT NOT CONFIRMED", "impact"); transformation.appendChild(before); const arrow = document.createElement("div"); arrow.className = "arrow"; text(arrow, "span", "↓"); text(arrow, "small", "remediation"); transformation.appendChild(arrow); const after = document.createElement("section"); after.className = "transform-card after"; text(after, "h2", "EXACT REPLAY"); text(after, "div", identity.action || baseline.action || "NOT ESTABLISHED", "action"); text(after, "div", replay.decision ? replay.decision.toUpperCase() : "NOT ESTABLISHED", replay.decision ? "decision" : "decision long-status"); text(after, "div", replay.decision === "blocked" ? "NO SYNTHETIC IMPACT" : "NOT ESTABLISHED", "impact"); transformation.appendChild(after); const proof = document.createElement("div"); proof.className = "replay-proof"; text(proof, "strong", identity.fixture_sha256 ? "SAME FIXTURE ✓" : "SAME FIXTURE ×"); text(proof, "span", identity.fixture_sha256 ? "SHA-256 MATCHED" : "SHA-256 NOT MATCHED"); transformation.appendChild(proof); story.appendChild(transformation);
