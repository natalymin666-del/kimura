...
(() => {
  ...
  function render() {
    ...
    const root = document.createElement("main"); root.className = "shell " + (isPass ? "is-pass" : isPartial ? "is-partial" : isFailed ? "is-failed" : "is-waiting"); root.dataset.liveState = model.state;
  ...
  window.KimuraLive = {applyEvent, applySnapshot, getState: () => clone(model), getSequence: () => model.sequence, pollOnce, reconcile};
  render();
  if (!TEST_MODE) load();
})();
"""
...
def render_live_page_html(run_id: str, *, api_base: str = "") -> str:
    ...
    script = _SCRIPT.replace("__RUN_ID__", json.dumps(run_id)).replace("__API_BASE__", json.dumps(api_base))
