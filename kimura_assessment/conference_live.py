...
(() => {
  ...
  function render() {
    ...
    const final = document.createElement("section"); final.className = "final-state"; const outcome = document.createElement("div"); text(outcome, "div", "Assessment outcome", "eyebrow"); text(outcome, "h2", isPass ? "FIX VERIFIED" : isPartial ? "PARTIAL" : isFailed ? "FAILED" : model.state === "cleanup_completed" ? "CLEANUP COMPLETED" : unknown ? "UNAVAILABLE" : "ASSESSMENT IN PROGRESS"); final.appendChild(outcome); text(final, "p", connection === "reconnecting" ? "Connection interrupted. Preserving the last proven state." : isPass ? "Exact replay verified under the deny-only remediation." : unknown ? "No assessment run is available." : "Only evidence-backed progress is displayed.", "final-copy"); if (isPass || isPartial || isFailed) { const handoff = document.createElement("div"); handoff.className = "mobile-handoff"; text(handoff, "strong", "MOBILE REPORT"); const link = document.createElement("a"); link.href = window.location.origin + "/report/" + encodeURIComponent(RUN_ID); link.textContent = link.href; handoff.appendChild(link); story.appendChild(handoff); } story.appendChild(final); row.appendChild(story); content.appendChild(row);
