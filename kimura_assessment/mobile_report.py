...
def _pass_invariants(evidence: Mapping[str, Any]) -> bool:
    ...
    return (
        ...
        and replay == {"decision": "blocked", "executed": False, "synthetic_event_id": None, "ledger_count": 1, "baseline_ledger_count": 1}
...
def derive_mobile_report(snapshot: Mapping[str, Any], *, expected_run_id: str | None = None) -> MobileReport:
    ...
    replay_impact = None
    if replay:
        replay_impact = True if replay.get("executed") is True or replay.get("synthetic_event_id") else False if replay.get("executed") is False and replay.get("synthetic_event_id") is None else None
