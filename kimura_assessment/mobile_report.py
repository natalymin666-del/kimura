...
def _pass_invariants(evidence: Mapping[str, Any]) -> bool:
    ...
    return (
        ...
        and {key: value for key, value in replay.items() if key not in (set(_SCENARIO_FIELDS) | {"scenario_facts"})} == {"decision": "blocked", "executed": True, "synthetic_event_id": None, "ledger_count": 1, "baseline_ledger_count": 1}
...
def derive_mobile_report(snapshot: Mapping[str, Any], *, expected_run_id: str | None = None) -> MobileReport:
    ...
    scenario_facts: dict[str, Any] = {}
    for event_name in _SCENARIO_EVENTS:
        facts = _payload(evidence, event_name).get("scenario_facts")
        if isinstance(facts, Mapping):
            for key, value in facts.items():
                scenario_facts.setdefault(key, value)
