...
class CustomerHtmlReportTests(unittest.TestCase):
    def test_report_has_customer_sections_and_explicit_boundaries(self):
        result = run_customer_assessment(CustomerAssessmentConfig.from_dict(config_values()), today=lambda: __import__("datetime").date(2026, 8, 22), provider_factory=lambda _: FakeProvider(), preflight_writer=lambda _: None)
    ...
    def test_report_escapes_customer_text_and_has_no_external_dependencies(self):
        ...
        result = run_customer_assessment(CustomerAssessmentConfig.from_dict(values), today=lambda: __import__("datetime").date(2026, 8, 22), provider_factory=lambda _: FakeProvider(), preflight_writer=lambda _: None)
        html = render_customer_report(result.report, result.evidence)
